...
// 获取下一个目标文件的路径
const getNewPath = (path, fileName) => {
  const pathAlias = getPathAlias();
  for (const k of Object.keys(pathAlias)) {
    if (fileName.startsWith(k)) {
      fileName = fileName.replace(k, pathAlias[k]).replaceAll("//", "/");
      return getProjectPath() + fileName;
    }
  }
  return getParentPath(path) + fileName;
};
...
const getDefinitionOne = (path, define) => {
  ...
  const fileName = content.matchAll(pattern).next().value[1];
  path = getNewPath(path, fileName).replaceAll("/", "\\");
...
const parseInterface = (path, str) => {
  const res = {};
  const arr = str.replaceAll("?", "").split(/[;,]/g);
...
const typeToString = (key, type, path) => {
  ...
  switch (t) {
    ...
    default:
      return type.includes("|") ? parseEnum(type) : getDefineObj(path, type);
  }
};
