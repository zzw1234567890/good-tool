...
const getInterfaceDefinitions = (path, defines) => {
  ...
  defines.map(([define]) => {
    ...
    obj.columns = getInterfaceInfo(path, define) || getDefinitionOne(path, define);
    res[define] = obj;
  });
  ...
  for(const define in res){
    if(!res[define].url){
      window.showInformationMessage(`${define}对应的url没有找到，请检查stateId是否一致`);
    }
  }
  return res;
};
...
const getInterfaceInfo = (path, define) => {
  const content = getFileContent(path);
  const pattern = new RegExp(`(interface|enum|type) ${define}.*?\{(.*?)\}`, "gs");
  const match = content.matchAll(pattern).next();
  if (match.value) {
    // eslint-disable-next-line no-unused-vars
    const [_, type, value] = match.value;
    let str = includeInnerClass(content, value);
    str = str.replace(/\/\/.*[/\n|\r|\t| ]/g, "").replace(/"|'|\n|\r|\t| /g, "");
    return type === "interface" || type === "type" ? parseInterface(path, str) : parseEnum(str);
  }
};

// 兼容匿名内部类
const includeInnerClass = (content, value) => {
