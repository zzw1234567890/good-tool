...
const { getCurrentProjectPath } = require("../util");
...
const readConfigFilesSync = (path, proxy) => {
  try {
    ...
    for (const { source, target, stompWs, ws } of proxyConfig) {
      proxy.set(source, { target, stompWs, ws });
    }
...
const switchEnv = async () => {
  const path = getProxyConfigPathByProjectPath(getCurrentProjectPath());
  const proxyConfig = JSON.parse(fs.readFileSync(path).toString());
